...
class IntegrationRuntime:
    ...
    def group_for_source(self, source_id: str) -> PhysicalGroup | None:
        return next(
            (group for group in self.groups if source_id in group.source_ids),
            None,
        )
    ...
    def controller_for_group(self, key: str):
        return next(
            (item for item in self.controllers.values() if item.group.key == key),
            None,
        )
    ...
    def device_info(self, group: PhysicalGroup) -> DeviceInfo:
        ...
        model = (
            native_device.model
            if native_device is not None and native_device.model
            else (
                "Unified TV Controller"
                if group.is_tv
                else "Unified Cast Controller"
            )
        )
        return DeviceInfo(
            identifiers={(DOMAIN, group.key)},
            entry_type=None,
            name=group.name or "Media device",
            manufacturer=manufacturer,
            model=model,
            configuration_url=(
                "https://github.com/Togarriapa/"
                "HomeAssistant-Cast-Metadata-Controls"
            ),
        )
    ...
    def health(self, group: PhysicalGroup) -> tuple[str, dict[str, Any]]:
        ...
        stale_routes = [
            capability
            for capability, source_id in configured_routes.items()
        ...
        unavailable_routes = [
            ...
            and self.manager.get_source(source_id) is not None
            and not self.manager.available(source_id)
        ]
        if not available:
            status = "unavailable"
        elif stale_routes or unavailable_routes:
            status = "degraded"
        else:
            # Optional companion sources can be unavailable while the physical TV
            # remains fully controllable through another representation.
            status = "healthy"
        return status, {
            ...
            "available_sources": [
                self.manager.get_entity_id(item) for item in available
            ],
            "unavailable_sources": [
    ...
    def _async_topology_changed(self) -> None:
        ...
        fingerprint = tuple(
            (group.key, group.source_ids) for group in candidate
        )
